...
- (void)dealloc
{
	[playback removeObserver:self forKeyPath:@"sourcePos1"];
...
{	
	...
	[playback addObserver:self forKeyPath:@"sourcePos1" options:NSKeyValueObservingOptionNew context:NULL];
...
- (void)initializeContents
{
	...
	_speaker_off = [[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"speaker_resized" ofType:@"png"]] CGImage];
	CGImageRetain(_speaker_off);
	_speaker_on = [[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"speaker_resized" ofType:@"png"]] CGImage];
